(function () {
  ...
  function renderCountdown(animate) {
    ...
    var birthday = days === 0;
    var progress = Math.round(currentState.progress * 10) / 10;
    ...
    setText('#buttonLabel', birthday ? 'おいわいれっしゃ しゅっぱつ！' : 'おして しゅっぱつ！');
    setText('#buttonHint', birthday ? 'おたんじょうび おめでとう！' : 'あと' + days + 'にちを はこぼう');
    setText('#rideNote', currentState.revealed ? 'きょうの 1えき、とうちゃくずみ！ なんどでも はしれるよ' : 'ボタンを おすと、でんしゃが はしるよ');
    ...
    if ($('#miniTrain')) $('#miniTrain').style.left = progress + '%';
    if (track) {
      track.setAttribute('aria-valuenow', String(Math.round(progress)));
      track.setAttribute('aria-valuetext', birthday ? 'たんじょうび駅に到着' : 'たんじょうび駅まであと' + days + '日');
      if (animate) {
    ...
    return afterTwoFrames().then(function () {
      ...
      if (!reducedMotion) {
        animateElement(journeyBackground, [
          ...
          { transform: type.id === 'express' ? 'scale(1.16) translate3d(-3.2%, 0, 0)' : 'scale(1.12) translate3d(-2%, 0, 0)' }
      ...
      var arrivalTransform = reducedMotion ? 'translate3d(-50%, 0, 0) scale(.96)' : 'translate3d(-43%, 2px, 0) scale(.94)';
      ...
      animateElement(trainShadow, [
        { transform: 'translateX(-50%) scale(1)', opacity: '.48' },
        { transform: reducedMotion ? 'translateX(-50%) scale(.9)' : 'translateX(-43%) scale(.88)', opacity: '.34' }
  ...
  function showJourneyFallback(type, rideState) {
    ...
    journeyVehicle.style.transform = 'translate3d(-46%, 0, 0) scale(.95)';
    setText('#showKicker', 'たんじょうびえき');
    setText('#showCountLabel', rideState.actualDays === 0 ? 'きょうは' : 'きょうは あと');
    setText('#showDaysNumber', rideState.actualDays === 0 ? 'おたんじょうび！' : String(rideState.actualDays));
    setText('#showCountUnit', rideState.actualDays === 0 ? '' : 'にち');
  ...
  function startJourney(useNextTrain) {
    if (journeyState === 'preparing' || journeyState === 'running' || journeyState === 'arriving') return;
    if (useNextTrain) updateTrainSelection(selectedTrainIndex + 1);
  ...
  function initialize() {
    ...
    if (!app || !journeyShow || !journeyVehicle || !journeyTrain || !$('#departButton') || !$('#routeRemaining')) {
